...
- **JWT**: Decode and validate Bearer tokens on protected routes (`/api/auth/me`, `/api/v1/documents`, `/api/v1/ai-usage`). The auth-service issues tokens with `iss: "algovn"`; Kong validates using the same `JWT_SECRET` from `.env`.
...
3. Call APIs via Kong:
   ...
   - Protected: `Authorization: Bearer <access_token>` for `GET http://localhost:8000/api/auth/me`, `http://localhost:8000/api/v1/documents`, etc.
